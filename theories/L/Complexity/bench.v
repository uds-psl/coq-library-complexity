...
From Undecidability Require Import Synthetic PolyTimeComputable.
...
Section explicit_bounds. 
  ...
  (*Check term_list_in_decb. *)
  (*Print list_in_decb_time. *)
  Section fixXeq. 
    Variable (X : Type).
...
Section uptoc_pure. 
  Section fixXeq. 
    ...
    *)
    ...
    (* NOTE: critique: here we essentially inline the running time of the previous thing *)
    ...
    Proof. 
      ...
      { extract. solverec; cycle 1. 
        + rewrite UpToC_le.
          ...
          Instead, we make use of set_consts to introduce local names to refer to constants appearing in the goal. 
          *)
          set_consts. inst c1 with (21 + C). nia.  
        + subst c1. unfold list_incl_decb_time. lia.
      } 
      smpl_upToC_solve. 
    Qed.
    Instance term_list_incl_decb : computableTime' (@list_incl_decb X eqbX) _ := projT2 _term_list_incl_decb. 
  End fixXeq. 
  Existing Instance term_list_in_decb. 
...
Section uptoc_poly. 
  ...
  Section existsb. 
    ...
    Fail Fixpoint existsb_time {X : Type} `{registered X} (p : (X -> nat) * list X) := let '(fT, l) := p in 
      match l with 
      | [] => 1 
...
(* as soon as a direct bound becomes ugly and we'd need to use it to bound another function, we instead first prove a polynomial bound and save us the effort of deriving an "exact" (up to constants) bound *)
Module uptoc_mixed. 
Section uptoc_mixed.
  (** extraction of list_in_decb *)
  Section fixXeq. 
...
Section fixX. 
    ...
    Context {Xeq : eqbClass eqbX}. 
    Context {XeqbComp : eqbCompT X}. 
    Lemma pTC_list_in_decb (Y : Type) `{registered Y} (f1 : Y -> list X) (f2 : Y -> X) : 
    ...
    Lemma pTC_list_inc_decb (Y : Type) `{registered Y} (f1 f2: Y -> list X):
      polyTimeComputable f1 
      -> polyTimeComputable f2 
  ...
  (* evalVar *)
  Lemma pTC_evalVar (Y : Type) `{registered Y} (f1 : Y -> assgn) (f2 : Y -> var) : 
    polyTimeComputable f1
    -> polyTimeComputable f2 
  ...
  (* evalLiteral *)
  Lemma pTC_evalLiteral (Y : Type) `{registered Y} (f1 : Y -> assgn) (f2 : Y -> literal) : 
    polyTimeComputable f1 
    -> polyTimeComputable f2 
