...
Definition HaltsOrDiverges_fixed_mTM (sig : finType) `{registered sig} n (M : TM sig (S n)) : Vector.t (tape sig) n * nat * nat -> Prop :=
...
Definition TMGenNP_fixed_mTM (sig : finType) `{registered sig} n (M : TM sig (S n))
  := (fun '(ts, maxSize, steps) =>
...
Definition TMGenNP_fixed_singleTapeTM (sig : finType) `{registered sig} (M : TM sig 1)
  := (fun '(ts, maxSize, steps) =>
