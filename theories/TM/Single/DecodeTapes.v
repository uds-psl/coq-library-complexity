...
Module CheckEncodesTape.
  Section checkEncodesTape.
    ...
    Context (sig : Type) (tau:finType) (n:nat) {I : Retract (sigTape sig) tau}.

    Remove Hints Retract_id : typeclass_instances.
    ...
    Proof.
      ...
      -cbn in Hf. rewrite !orb_true_r in Hf. subst f. revert Hres. 
       ...
       +intros ? ? ? ? -> [= ->] (Hneq&Hsym&(?&Hlast)&Hmarks&Hnempty)%encode_tape_invariants_partial;cbn in *.
        2:now setoid_rewrite <- in_rev.
       ...
       +intros ? ? ? ? -> [= -> ] (Hneq&Hsym&(?&Hlast)&Hmarks&?)%encode_tape_invariants_partial;cbn in *.
        2:now setoid_rewrite <- in_rev.
        destruct t__R1. 2:{ ediscriminate (Hsym (RightBlank _)). rewrite tl_app. 2:now length_not_eq. eauto. }
        destruct t__L'. 2:{ inversion H__seenSymbol. now apply H. }
        cbn in *. nia.
       +intros ? ? ? ? -> [= ->] (Hneq&Hsym&(?&Hlast)&Hmarks&?)%encode_tape_invariants_partial;cbn in *.
        2:now setoid_rewrite <- in_rev.
        destruct t__R1. 2:{ ediscriminate (Hsym (RightBlank _)). rewrite tl_app. 2:now length_not_eq. eauto. }
      ...
      -revert Hf Hres. cbn. destruct seenMark;cbn.
       ...
       { clear IH. cbn. rewrite f'_equation;cbn.  
         intros ->;cbn.
         intros ? ? ? ? Hnil [= ->] (Hneq&Hsym&(?&Hlast)&?&?)%encode_tape_invariants_partial;cbn in *.
         2:now setoid_rewrite <- in_rev.
       ...
       +destruct H' as (x&t__R1&t__R2&c'&->&Hc'&Hx&->).
        eexists x,(_::t__R1),t__R2,_.
        repeat (cbn in Hx|-*;autorewrite with list in Hx|-* ). erewrite <- retract_g_inv. 2:symmetry;eassumption.
        split. 2:split. 3:split.
        1,2:reflexivity. easy.
        destruct (rev (map f__I t__R1));cbn;now autorewrite with list.
       +intros x ? ? ? H__R [= ->] Hx. specialize encode_tape_invariants_partial with (1:=Hx) as (Hneq&Hsym&(?&Hlast)&_);cbn in *. now setoid_rewrite <- in_rev.
        ...
        destruct t__R1. now inv Heq. inv H__R. eapply H'. reflexivity.  now rewrite retract_g_adjoint.
        rewrite Hx;cbn;autorewrite with list;cbn. reflexivity.
      -revert Hf Hres. cbn. intros ->;cbn.
       destruct t__R.
       { clear IH. cbn. rewrite f'_equation;cbn.  
         intros ->;cbn.
         intros ? ? ? ? Hnil [= ->] (Hneq&Hsym&(?&Hlast)&?&?)%encode_tape_invariants_partial;cbn in *.
         2:now setoid_rewrite <- in_rev.  destruct t__R1. 2:easy. destruct t__R2. 2:easy. easy.
       }
       cbn;intros H.
       ...
       destruct res as [[] t'];cbn in *.
       +destruct H' as (x&t__R1&t__R2&c'&->&Hc&Hx&->).
        eexists x,(_::t__R1),t__R2,_. cbn.  erewrite <- retract_g_inv. 2:symmetry;eassumption.
        repeat (cbn in Hx|-*;autorewrite with list in Hx|-* ). split. 2:split. 3:split. 1,2:reflexivity. easy.
        destruct (rev (map f__I t__R1));cbn;now autorewrite with list.
       +intros x ? ? ? H__R [= ->] Hx. specialize encode_tape_invariants_partial with (1:=Hx) as (Hneq&Hsym&(?&Hlast)&_);cbn in *. now setoid_rewrite <- in_rev.
        destruct (rev t__R1) eqn:Heq. all:inv Hlast.
        destruct t__R1. now inv Heq. inv H__R. eapply H'. 1:reflexivity. 1:now rewrite retract_g_adjoint. rewrite Hx;cbn;autorewrite with list;cbn. reflexivity.
    Qed.
    ...
    Proof.
      ...
      2:{ intros [= <- <-];cbn;intros ? ? ?. destruct x;cbn;eexists;(split;[reflexivity| ]).
          all:intros ->;cbn in Hcur. all:rewrite retract_g_adjoint in Hcur;inv Hcur. }
      destruct t as [ | | | t__L s' t__R];cbn in Hcur. all:inversion Hcur.
      destruct isLeftBlank eqn:H__LB.
      2:destruct isNilBlank eqn:H__NB.
      2:{ intros [= <- <- ]. destruct s;inv H__NB. apply retract_g_inv in Hcur as ->. eexists (@niltape _),_,_;cbn. eauto 10. }
      2:{ intros [= <- <- ]. intros ? x ?. destruct x;cbn;eexists;(split;[reflexivity | ]).
          all:intros [=];subst. all:rewrite retract_g_adjoint in Hcur;inv Hcur. all:easy. }
      destruct s;inv H__LB.
      destruct t__R. 1:{ cbn. rewrite f'_equation. cbn. intros [= <- <-]. intros ? x ?. destruct x;cbn;eexists;(split;[reflexivity | ]). all:intros [= -> ->]. all:rewrite retract_g_adjoint in *;inv Hcur;inv H0.  all:revert H1. all:length_not_eq. }
      intros H';symmetry in H'. assert (H:=H'). revert H. cbn. rewrite (retract_g_inv Hcur) at 1. intros H%(f'_spec (t__L':=[])).
      2:now destruct marked;cbn;nia. 2:easy. 2:now constructor. destruct b;cbn  in H.
      -destruct H as (x&t__R1&t__R2&c'&->&Hc&Hx&->). symmetry in Hc. erewrite (retract_g_inv Hc).
       all:destruct (rev (encode_tape x)) eqn:Hlast.
       1:{rewrite Hx in Hlast. exfalso;revert Hlast; length_not_eq. }
       do 5 eexists.
       +rewrite Hx;cbn. erewrite (retract_g_inv Hcur). auto.
       +rewrite <- map_rev. setoid_rewrite Hlast at 1. rewrite Hx in Hlast. cbn in Hlast. split. easy.
        apply (f_equal (map f__I)) in Hlast. apply (f_equal (hd (f__I c'))) in Hlast.
        cbn in Hlast. rewrite map_app in Hlast. rewrite hd_app in Hlast. 2: length_not_eq.
        autorewrite with list in Hlast. rewrite Hx. rewrite map_rev in *.
        cbn. rewrite <- !tl_app. 2,3:length_not_eq. repeat (autorewrite with list in *;cbn in * ).
        congruence.
      -intros ? x ? . exists (f__I (hd NilBlank (encode_tape x))). split. now destruct x.
       intros [= <- -> H2]. rewrite retract_g_adjoint in Hcur. inv Hcur. destruct encode_tape eqn:Heqt. now inv H3.
       cbn in *. subst s. destruct l. 1:{ destruct x;cbn in Heqt;try now inv Heqt. all:revert Heqt. all:length_not_eq. }
       inv H2. eapply H. 3:exact Heqt. reflexivity. now rewrite retract_g_adjoint.
    Qed.
