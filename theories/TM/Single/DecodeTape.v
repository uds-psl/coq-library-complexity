...
Module CheckEncodesTape.
  Section checkEncodesTape.
    ...
    Import EncodeTapes StateWhile Mono Multi Copy Switch If Combinators Single.EncodeTapesInvariants.
    ...
    Proof.
      ...
      2:{ intros [= <- <-];cbn;intros ? ? ?. destruct x;cbn;eexists;(split;[reflexivity| ]). all:intros ->;cbn in Hcur. all:now rewrite retract_g_adjoint in Hcur. }
      ...
      destruct t__R. 1:{ cbn. rewrite f'_equation. cbn. intros [= <- <-]. intros ? x ?. destruct x;cbn;eexists;(split;[reflexivity | ]). all:intros [= <- ?%retract_f_injective ?]. easy. all:revert H0. all:length_not_eq. }
      ...
      -destruct H as (x&t__R1&t__R2&c'&->& ->%eq_sym%retract_g_inv &Hx&->). all:destruct ((rev (map Retr_f (encode_tape x)))) eqn:Hlast.
       ...
       +setoid_rewrite Hlast at 1. rewrite Hx in Hlast. cbn in Hlast.
        apply (f_equal (hd (Retr_f c'))) in Hlast. cbn in Hlast. rewrite hd_app in Hlast. 2: length_not_eq.
        rewrite Hlast.  rewrite Hx. cbn. rewrite <- !tl_app. 2,3:length_not_eq. repeat (autorewrite with list;cbn). eauto 10.
      -intros ? x ?. eexists (hd (Retr_f NilBlank) (map Retr_f (encode_tape x))). split. now destruct x.
       intros [= <- H1 H2]. destruct encode_tape eqn:Heqt. all:apply retract_f_injective in H1. now inv H1. cbn in *. subst s.
       destruct l. 1:{ destruct x;cbn in Heqt;try now inv Heqt. all:revert Heqt. all:length_not_eq. }
       inv H2. eapply H. reflexivity. 2:exact Heqt. now rewrite retract_g_adjoint.
    Qed.
