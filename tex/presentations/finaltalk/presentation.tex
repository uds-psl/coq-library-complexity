...
\begin{frame}{L for Complexity Theory\footnote{Definitions by Fabian}}
...
\begin{frame}{Basic Definitions: Polynomial-time Reductions}
  ...
  \onslide<3->{
    \begin{align*}
      \textsf{NP-hard}~P := \forall Q, Q \in \NP{} \rightarrow Q \redP{} P 
    \end{align*}
  }
\end{frame}
...
\begin{frame}{Encoding as a Boolean Formula}
  ...
  $\Phi \defeq \Phi_{\textsf{init}} \land \Phi_{\textsf{trans}} \land \Phi_{\textsf{final}}$
  \begin{align*}
...
\begin{frame}{Formulas: Representation of Predicates}
  \[f : \formula \defeq \btrue \bnfmid v \bnfmid f_1 \lor f_2 \bnfmid f_1 \land f_2 \bnfmid \lnot f \qquad (v : \nat) \]
    ...
    \only<5->{ 
      ...
      \onslide<6->{
        \begin{gather*}
          ...
          \only<7-> { 
            N \defeq \underbrace{N_1}_{f_1 \leftrightarrow v_1} \concat{} \underbrace{N_2}_{f_2 \leftrightarrow v_2} \concat{} \alt<-7>{?\phantom{(\leftrightarrow (v_1 \lor v_2))}}{(v \leftrightarrow (v_1 \lor v_2))} \\ 
