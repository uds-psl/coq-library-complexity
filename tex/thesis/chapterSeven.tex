...
A rather trivial but important fact is that \encodesPred{} is congruent with respect to the encoded predicate. This will be quite convenient for proving correctness of encodings by switching to an equivalent characterisation of the predicate we want to encode.
...
For our convenience, we define $m \defeq \length{x_0}$ to be the length of each of the tableau's lines. \mnote{$m$}
