...
Nowadays, the question whether P = NP, i.e.\ whether every problem which is verifiable in polynomial time is also decidable in polynomial time, is one of the biggest open problems of computer science\cite{TODO}. If one were to find a polynomial-time decider for an NP-hard problem such as \SAT{}, one could directly answer ``P = NP!''. \todo{maybe introduce NP-completeness} 
...
The theoretical foundation for using L for the formalisation of complexity theory has been laid by Forster et al.\ in~\cite{TODO}. There it is shown that Turing machines and L can simulate each other with a polynomial overhead in time and a linear overhead in space for decision problems with at least linear space usage, if one chooses the right resource measures for L.\todo{mention here that the measures will be introduced in chapter 2?}
This result is crucial as it proves that the basic complexity classes for Turing machines and L corresond; in particular, problems which are NP-hard for L are also NP-hard for Turing machines.\todo{this is somewhat imprecise as it really depends on the definition of NP-hardness}
