...
\setCoqFilename{Undecidability.L.Complexity.Cook.Prelim}
...
\begin{proposition}\label{prop:relpower}
  ...
  \begin{enumerate}
    \coqitem[relpower_trans] Transitivity: $R^n~x~y \rightarrow R^m~y~z \rightarrow R^{n+m}~x~z$ and $\prescript{n}{}R~x~y \rightarrow \prescript{m}{}R~y~z \rightarrow \prescript{n+m}{}R~x~z$
...
\begin{definition}[Turing machines]
  Given a finite type $\Sigma$ and a number of tapes $n$, Turing machines of type \mnotec[mTM]{$\textsf{TM}~\Sigma~n$} are tuples $(Q, \delta, \textsf{start}, \textsf{halt})$, where $Q$ is the finite type of states, $\delta : Q \times {(\opt{\Sigma})}^n \rightarrow Q \times {\textsf{Act}_\Sigma}^n$ is the transition function, $\textsf{start}$ is the initial state and $\textsf{halt} : Q \rightarrow \bool$ defines the halting states.
\end{definition}
...
\setCoqFilename{Undecidability.L.Complexity.Cook.TM_single}
...
\begin{definition}[\NP{} hardness and \NP{} completeness]
  A problem $Q : X \rightarrow \Prop$ is \coqlink[NPhard]{\NP{}-hard} if any problem $P : Y \rightarrow \Prop$ can be reduced to it in polynomial time, i.e.\ $P \redP{} Q$. It is \coqlink[NPcomplete]{\NP{}-complete} if, in addition, $Q \in \normalfont\NP{}$.
\end{definition}
