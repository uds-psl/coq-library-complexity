...
As we want to do complexity theory in L, it we have to define time and space measures for L.
...
\begin{definition}[Polynomial-time Computable Functions]
  Assume types $X, Y$ which are L-encodable.\footnote{This does, for instance, include all inductive datatypes for which there is a Scott encoding.}
...
\subsection{\textsf{P} and \textsf{NP}}
...
\begin{definition}[Polynomial-time Decidable Problems]
  Let $p : X \rightarrow \Prop$ for an L-encodable type $X$. $p$ is polynomial-time decidable if there exists a function $f_p : \nat \rightarrow \nat$ such that $p$ is decidable in time $f_p$ and $f_p$ is monotonic as well as polynomially bounded. 
\end{definition}
...
For \NP{}, we do not use the usual definition via nondeterminism. While nondeterministic additions to the $\lambda$ calculus have been explored in the literature, for instance via the addition of a new combinator allowing to nondeterministically guess a single bit, using nondeterminism would make formal reasoning much harder.\todo{citations}
Instead, we adapt the well-known alternative verifier characterisation of \NP{}, where there must exists a verifier which decides whether a certificate of polynomial size correctly proves that an instance is a yes-instance. Intuitively, the verifier characterisation moves the nondeterminism into the input.
\begin{definition}[\NP{}]
  ...
  \begin{itemize}
    ...
    \item and there exists a monotonic and polynomially-bounded $f_p : \nat \rightarrow \nat$ bounding the size of certificates, i.e. 
      \begin{itemize}
...
Note that we require that the type of certificates is $\Lterm$. This does not pose a restriction in practice as we can still use any $L$-encodable type (since these types can be encoded as L terms) whose encoding can be decoded in polynomial time, i.e.\ for which there exists a function $f : \Lterm \rightarrow \opt{X}$ running in polynomial time. It turns out that the Scott encoding of any inductive datatype can in practice be decoded in linear time.
...
Now, since $p$ is \NP{}-hard, we can reduce the computation of L terms to $p$. By transititivity of reduction, we get a polynomial-time overhead simulation of L using Turing machines. The two combined reductions thus solve L's size explosion problem. 
...
Motivated by this reasoning, we propose to use Turing machines as an intermediate problem for proving a natural problem to be \NP{}-hard: Turing machines are much more expressive than most natural problems, thus they seem to be a good candidate for solving the size explosion problem.
