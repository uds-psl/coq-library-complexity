...
\begin{definition}[\NP{}-hardness and \NP{}-completeness]
  A problem $Q : X \rightarrow \Prop$ is \coqlink[NPhard]{\NP{}-hard} if any problem $P : Y \rightarrow \Prop$, where the encoding $\lambda~x. \overline{x}$ of $Y$ can be computed in polynomial time, can be reduced to it in polynomial time, i.e.\ $P \redP{} Q$. It is \coqlink[NPcomplete]{\NP{}-complete} if, in addition, $Q \in \normalfont\NP{}$.
\end{definition}
