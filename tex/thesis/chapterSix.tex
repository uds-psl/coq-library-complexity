...
\setCoqFilename{Undecidability.L.Complexity.Cook.UniformHomomorphisms}
...
\begin{proposition}
  ...
  \begin{enumerate}
    \coqitem[canonicalHom_is_homomorphism] $\homomorphism~(\canonicalHom~f)$
    \coqitem[canonicalHom_is_unique] $\forall h, \homomorphism~h \rightarrow (\forall x, h [x] = f~x) \rightarrow h \equiv \canonicalHom~f$
  \end{enumerate}
\end{proposition}
...
\setCoqFilename{Undecidability.L.Complexity.Cook.PR_homomorphisms}
...
\begin{lemma}[Agreement of $\textsf{rewHead}$]\label{lem:rewHead_agree}\leavevmode
  \begin{enumerate}
    ...
    \coqitem[rewritesHead_homomorphism2] If $w \in \textsf{hwindows}$, $\length{a_1} = o$, $\length{u} = k \cdot o$, and $\rewHead{w}{(h~a_1 \concat h~a_2)}{(u \concat v)}$, then there exists $b_1$ with $u = h~b_1$ and $\length{b_1} = o$.
  \end{enumerate}
\end{lemma}
\begin{proof}
...
\setCoqFilename{Undecidability.L.Complexity.Cook.PR_to_BinaryPR}
...
\begin{fact}[Uniformity]\leavevmode
  \begin{enumerate}
    ...
    \coqitem[h'_length] $\length{h_{\Sigma}~\sigma} = \length{\Sigma}$
  \end{enumerate}
\end{fact}
...
\begin{fact}[Injectivity]\leavevmode
  \begin{enumerate}
    ...
    \coqitem[h'_injective] $\textsf{injective}~h_{\Sigma}$
  \end{enumerate}
\end{fact}
...
\setCoqFilename{Undecidability.L.Complexity.Cook.FlatPR_to_BinaryPR}
