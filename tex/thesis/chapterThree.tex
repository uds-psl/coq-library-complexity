...
\subsection{Running-time Analysis Using the Extraction Mechanism}
  ...
  In the case of $\eval_N$, the polynomial $p_{\eval_N}~n \defeq n \cdot p_{\eval_C}~n + (n +1) \cdot c_{\eval_N}$ works, where $p_{\eval_C}$ is the polynomial bounding $T(\eval_C)$ and $c_{\eval_N}$ is a constant. The bounding equation is then proved by induction on the CNF $N$. 
  ...
  Recall that, regarding the size, we only need to bound the size of the result. The procedure of deriving a polynomial is similar in that case.
