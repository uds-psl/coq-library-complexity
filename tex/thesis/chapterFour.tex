...
Recall that, in order to show a language $L$ to be \NP{}-hard, we have to show that any language $L'$ contained in \NP{} can be reduced to it. In our setting of complexity theory in L, we thus have to reduce the computation of L to \SAT{}.
...
\gennp{} is a form of a generic problem for Turing machines. A definition the reader might be more familiar with is the following: 
\begin{center}
  Given a nondeterministic Turing machine $M$, an input $in$ and a number of steps $t$, does $M$ accept $in$ in at most $t$ steps?
\end{center}
Our definition relates to this one in a similar way the verifier characterisation of \NP{} relates to the one using nondeterminism: the nondeterminism is moved into the input. The reason for that simply is that formally working with nondeterminism is unpleasant.
Additionally, we have made the simplification that the input of the Turing machine consists only of the nondeterministic certificate, as the ``regular'' input can just be hardcoded into the Turing machine. 
...
In this setting, we just see \gennp{} the same as any other problem; one could even consider it to be natural as it does not refer to our model of computation.

\section{Deterministic Simulation: Tableau of Configurations}
We first consider how a deterministic Turing machine can be encoded.
...
\begin{example}[Moving Head]\label{ex:movinghead}
  ...
  The successor of the tableau line using moving-head semantics
  \begin{center}
    \begin{tikzpicture}
...
\begin{center}
  \begin{tikzpicture}
    ...
    \draw[->, dashed] (3.75, -0.75) -- (4.5, -1.5);
    \draw[->, dashed] (6.75, -0.75) -- (7.5, -1.5);
...
\begin{example}[Moving Tape]\label{ex:movingtape}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  Note that the size of the substrings left and right of the center symbol is the same number $z$ depending on $t$ and $k$, expressing that the state symbol is fixed at the center.
\end{example}
...
These rewrite windows are used to justify that one line of the tableau \emph{validly} follows from the previous one. At each possible offset, there must exist a rewrite window whose premise matches a prefix of the previous string and whose conclusion matches prefix of the succeding string.
...
Seeing this generaliation, one might now realise a problem regarding the tape shifts: the rewrite windows are not provided enough information such that the overlapping constraints can enforce a consistent tape shift.
\begin{example}[Inconsistent Tape Shifts]
  ...
  \end{center}
  Looking again at Example~\ref{ex:movingtape}, this rule can be applied at the position labelled with 1 to obtain the an alternative successor string:
  \begin{center}
    \begin{tikzpicture}
      \draw (0, 0.75) -- (9.75, 0.75);
      \draw (0, 0) -- (9.75, 0);
...
Each symbol of the tape alphabet is annotated with a polarity indicating in which direction the tape is shifted. They do not add any information to the configuration strings in their own right and are just relevant for the configuration changes.
Notationally, we write $\polpos{\sigma}, \polneg{\sigma}, \polneut{\sigma}$ for a symbol $\sigma$ annotated with a positive, negative, or neutral polarity. Blanks do not need polarities.
...
Formally, for each of the rewrite rules depicted in Example~\ref{ex:polarities}, do an instantiation not only for all possible combinations of variable valuations, but also for the three possible polarities in the premise (we only require that the polarities are consistent).

Finally, we need to enforce that the final configuration is a halting configuration. This can be encoded by requiring that an element of $\{q^m | \textsf{halt}~q = \btrue\}$ is contained in the last line of the tableau.

\section{Nondeterministic Input}
...
The reduction of \gennp{} to Parallel Rewriting (\PR{}), a string-based problem which will be introduced in Section~\ref{TODO}, handles the main part of the Turing machine simulation. After that, we do not have to reason about Turing machines anymore. 
Parallel Rewriting can be seen as a problem between Turing machines and circuits.
