...
\begin{definition}[Matching Windows]\label{def:rewHead}
  \mnote[rewritesHead]{\textsf{rewHead}}
...
\begin{definition}[Validity (Explicit Characterisation)]
  Given a set of rewrite windows $R$, the validity of a rewrite of $a : \str{\Sigma}$ to $b : \str{\Sigma}$, written \mnotec[validExplicit]{$a \strentE{R} b$}, is defined by:
  \begin{align*}
...
\begin{definition}[Validity] 
  Given a set of rewrite windows $R$, the validity of a rewrite of $a : \str{\Sigma}$ to $b : \str{\Sigma}$, written \mnotec[valid]{$a \strent{R} b$}, is defined inductively:  
  \begin{gather*}
    \infer{\nil \strent{R} \nil}{} \\
...
\begin{proof}
  ...
  \begin{enumerate}[1)]
    \coqitem[relpower_inr_liftPrelude] If $n \le t'$ and $x_0 \strent{\Rcomb}^n b$, then $x_0 \strent{R'} b$. 
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node (flatpr) at (1, 0) {\textbf{Flat-3-PR}};
    \node (tm) at (0, 1) {\gennp{}};
    \node (pr) at (1, 1) {\textbf{3-PR}};
    \path[dashed, ->] (flattm) -- (tm);
    \path[dashed, ->] (tm) -- (pr);
    \path[dashed, ->] (pr) -- (flatpr);
    \path[->] (flattm) -- (flatpr);
  \end{tikzpicture}
\end{center}
