...
\setCoqFilename{Undecidability.L.Complexity.Problems.Cook.GenNP}
\begin{definition}[TMGenNP][SingleTMGenNP]\label{def:tmgennp}
  \mnote[SingleTMGenNP]{TMGenNP}
  \begin{align*}
  ...
  and 
  \setCoqFilename{Undecidability.L.Complexity.Problems.GenNP.TMGenNP_fixed_mTM}
  \begin{align*}
...
\begin{definition}[Parallel Rewriting instances][PR]\label{def:pr}
  \mnote[PR]{\PR{} instances}
  ...
  \end{itemize}
  \todo{move to lists instead of vectors}
...
Instead of directly using the projections $\pi_1$ and $\pi_2$ on a window $w$, we usually write $\prem{w}$ and $\conc{w}$\mnote{$\textsf{prem}, \textsf{conc}$}. In the previous chapter, we considered rewrite windows of width 3.
...
\begin{definition}[Validity, Explicit Characterisation][validExplicit]\label{def:valid_exp}
  Given a set of rewrite windows $R$, the validity of a rewrite from $a : \str{\Sigma}$ to $b : \str{\Sigma}$, written \mnotec[validExplicit]{$a \strentE{R} b$}, can be defined by:
  \begin{align*}
...
\begin{definition}[Validity][valid]
  Given a set of rewrite windows $R$, the validity of a rewrite from $a : \str{\Sigma}$ to $b : \str{\Sigma}$, written \mnotec[valid]{$a \strent{R} b$}, is defined inductively:  
  \begin{gather*}
    \infer{\nil \strent{R} \nil}{} \\
...
\begin{proposition}[Vacuous Rewriting][valid_vacuous]\label{lem:vacuous}
...
\begin{proposition}[Length Invariance][valid_length_inv]
  Let $a, b : \str{\Sigma}$ with $a \strent{} b$. Then $\length{a} = \length{b}$. 
\end{proposition}

\begin{lemma}[Agreement of $\strentE{}$ and $\strent{}$][valid_iff]\label{lem:agree_valid}
...
\begin{definition}[Substring constraint][satFinal]
  Given a set of strings $\Rfinal : \listsof{\str{\Sigma}}$, string $s$ satisfies $\Rfinal$, written \mnotec[satFinal]{$s \models \Rfinal$}, if:
...
\begin{definition}[Parallel Rewriting][PRLang]
  \mnote[PRLang]{\PR{}}
...
\setCoqFilename{Undecidability.L.Complexity.Problems.Problems.Cook.TPR}
...
\setCoqFilename{Undecidability.L.Complexity.Reductions.Cook.SingleTMGenNP_to_TPR}
\begin{align*}
...
\begin{definition}[Polarity Reversion]
  ...
  \end{align*}
  Notationally, we write \mnotec[polarityFlip]{$\pFlipi{p}$} for $\pFlip{p}$. 
\end{definition}
...
\begin{fact}[Involutions]\label{fact:prev_involution}
  $\pFlip{}$ and $\pRev{}$ are involutions, that is, \coqlink[polarityFlip_involution]{$\pFlip{(\pFlip{p})} = p$} and \coqlink[polarityRev_involution]{$\pRev{(\pRev~h)} = h$}.
\end{fact}
...
\begin{proposition}\label{prop:tapefacts}\leavevmode
  ...
  \begin{enumerate}
    ...
    %\coqitem[string_reprTapeP_length] $u \reprtt{n}{p} h \rightarrow \length{h} = \natS{\hat{n}}$
    \coqitem[tape_repr_polarityFlip] $u \reprtt{n}{p} h \rightarrow u \reprtt{n}{\pFlipi{p}} \withl \pFlipi{\gamma} \withm \gamma \in h \withr$
  \end{enumerate}
\end{proposition}
...
Both $\reprt{}$ and $\reprc{}$ are computational \mnote[stringForTapeHalf]{$\stringForTapeHalf$}in the sense that, given a tape half or a configuration, respectively, we can compute a string representing this tape half or configuration. 
...
\begin{center}
  ...
  \centerTheFuckingText{(both halves)}
  \\
...
\centerTheFuckingText{(right half)} \\
...
\centerTheFuckingText{(left half)}
\end{center}
...
\begin{lemma}[Symmetry of \Rtape][tapeRules_revp]\label{lem:symm_rtape}
...
\begin{lemma}[Symmetry of Tape Rewrites]\label{lem:symm_tape_rew}\leavevmode
  \begin{enumerate}
    \coqitem[tape_rewrite_symm1] $h \strent{\Rtape} h' \rightarrow \pRev~h \strent{\Rtape} \pRev~h'$ 
    \coqitem[tape_rewrite_symm2] $\pRev~h \strent{\Rtape} \pRev~h' \rightarrow h \strent{\Rtape} h'$
    \coqitem[tape_rewrite_symm3] $h \strent{\Rtape} h' \rightarrow \withl \pFlipi{x} \withm x \in h \withr \strent{\Rtape} h'$
  \end{enumerate}
\end{lemma}
\begin{proof}
  \begin{enumerate}
...
%\paragraph{Generalisation to Arbitrary Tape Halves}
...
%special cases for the empty tape proved previously for the base case.

\begin{lemma}[Adding a Symbol]\label{lem:tape_add}
  If $rs \reprtt{n}{p} h$ and $\length{rs} < n$, then 
  \begin{enumerate}
    \coqitem[tape_repr_add_right] $\mexists h', h \strent{\Rtape} \polpos{\sigma} :: h' \land \sigma :: rs \reprtt{n}{+} \polpos{\sigma} :: h'$ 
    \coqitem[tape_repr_add_left] $\mexists h', \rev~h \strent{\Rtape} \rev{(\polneg{\sigma} :: h')} \land \sigma :: rs \reprtt{n}{-} \polneg{\sigma} :: h'$
  \end{enumerate}
\end{lemma}
\begin{proof}
...
\begin{lemma}[Removing a Symbol]\label{lem:tape_rem}
  ...
  \begin{enumerate}
    \coqitem[tape_repr_rem_right] $\mexists h', \sigma^p :: m^p :: h \strent{\Rtape} \polneg{m} :: h' \land rs \reprtt{n}{-} \polneg{m} :: h'$
    \coqitem[tape_repr_rem_left] $\mexists h', \rev{(\sigma^p :: m^p :: h)} \strent{\Rtape} \rev{(\polpos{m} :: h')} \land rs \reprtt{n}{+} \polpos{m} :: h'$
  \end{enumerate}
\end{lemma}
...
\begin{lemma}[Leaving the Tape Unchanged]\label{lem:tape_stay}
  ...
  \begin{enumerate}
    \coqitem[tape_repr_stay_right] $\mexists h', m^p :: h \strent{\Rtape} \polneut{m} :: h' \land rs \reprtt{n}{\circ} \polneut{m} :: h'$
    \coqitem[tape_repr_stay_left] $\mexists h', \rev{(m^p :: h)} \strent{\Rtape} \rev{(\polneut{m} :: h')} \land rs \reprtt{n}{\circ} \polneut{m} :: h'$
  \end{enumerate}
\end{lemma}
...
%for instance, if the tape is currently $\textsf{leftof}~\sigma~rs$ and $\delta(q, \ONone) = (p, \ONone, \movel)$, the successor tape is $\textsf{leftof}~\sigma~rs$ again.
...
\begin{definition}[State Symbols][isSpecStateSym]
...
\begin{lemma}[Tape Rewriting]\label{lem:sim_inv1}\leavevmode
  \begin{enumerate}
    \coqitem[rewHeadTrans_tape'] $u \reprtt{n}{p} h \rightarrow (\exists w \in \Rsim, \rewHead{w}{h}{h'}) \rightarrow (\exists w \in \Rtape, \rewHead{w}{h}{h'})$
    \coqitem[rewHeadSim_tape] $u \reprtt{n}{p} h \rightarrow h \strent{\Rsim} h' \rightarrow h \strent{\Rtape} h'$
    \coqitem[rewHeadSim_tape_polarityRev] $u \reprtt{n}{p} h \rightarrow (\pRev~{h}) \strent{\Rsim} (\pRev~{h'}) \rightarrow (\pRev~{h}) \strent{\Rtape} (\pRev~{h'})$
  \end{enumerate}
\end{lemma}
\begin{proof}
  \begin{enumerate}
...
\begin{lemma}[State Rules]\label{lem:sim_inv2}\leavevmode
  ...
  Then:
  \begin{enumerate}
    \coqitem[rewHeadSim_trans] $\textsf{halt}~q = \bfalse \rightarrow w \in \Rsim \rightarrow w \in \Rtrans$
    \coqitem[rewHeadSim_halt] $\textsf{halt}~q = \btrue \rightarrow  w \in \Rsim \rightarrow w \in \Rhalt$
  \end{enumerate}
\end{lemma}
...
\begin{proposition}[][valid_rewritesHeadInd_center]\label{lem:rewrite_split}
  \begin{align*}
...
\begin{proposition}[][valid_conc_inv]\label{prop:rewrite_split_conv}
  \begin{align*}
...
\begin{lemma}[Step Simulation][stepsim]\label{lem:stepsim}~\\
...
\begin{lemma}[Halting Simulation][haltsim]\label{lem:haltsim}~\\
...
\begin{lemma}[Multi-step Completeness][multistep_simulation]\label{lem:multistep_complete}
...
\begin{lemma}[Multi-step Halting][multistep_halt]\label{lem:multistep_halt}
  If $(q, tp) \reprc{} s$, $\textsf{halt}~q = \btrue$, then $\mexists s', s \strent{\Rsim}^i s' \land (q, tp) \reprc{} s'$. 
\end{lemma}

\begin{lemma}[Multi-step Soundness][multistep_inversion]\label{lem:multistep_sound}
  If $(q, tp) \reprc{} s$, $i \le z$, $\length{tp} \le z - i$,\todo{$i \le t$} and $s~\strent{\Rsim}^i~s'$, then there exist $q', tp'$ and $j$ with 
  \begin{itemize}
    \item $(q', tp') \reprc{} s'$, 
    \item $j \le i$, 
...
\begin{definition}[Halting String][containsHaltingState]
...
\begin{definition}[Final Substrings][finalSubstrings]
  \mnote[finalSubstrings]{$\Rfinal$}
  $\Rfinal \defeq \withl q^m \withm \textsf{halt}~q = \btrue \withr$
\end{definition}

\begin{proposition}[][finalSubstrings_correct]
  $s \models \Rfinal \leftrightarrow \haltString{s}$
\end{proposition}

We have now \coqlink[simulation]{reduced} \gennp{} to the following question: 
\begin{problem}[][TM_reduction_to_ExPTPR]\label{prob:exinput}~\\
...
\setCoqFilename{Undecidability.L.Complexity.Reductions.Cook.PTPR_Preludes}
...
The alphabet of the new \PR{} instance will be $\mathcal{A} \defeq \Gamma + \Delta$.\mnote[combSigma]{$\mathcal{A}$}
...
\begin{assumption}[Structural Assumptions on the Prelude]
  ...
  \end{description}
\end{assumption}
\todo{the setup in the mechanisation is a bit different because we prove a more general statement}
...
\begin{lemma}[][relpower_comb_split]\label{lem:prelude_split}
...
\begin{proof}
  ...
  \begin{enumerate}[1)]
    \coqitem[relpower_inr_liftPrelude] If $n \le t'$ and $x_0 \strent{\Rcomb}^n b$, then $x_0 \strent{R'}^n b$. 
    \coqitem[relpower_inl_liftOrig] If $\length{s} \ge 3$, $\isOrigString~s$, and $s \strent{\Rcomb}^n b$, then $s \strent{R}^n b$ and $\isOrigString~b$. 
  \end{enumerate}
...
\begin{remark}
  ...
  In the mechanisation, we \coqlink[red_to_exptpr]{prove this more general statement}, but we do not need it for the Turing machine simulation.
\end{remark}
...
As in the previous section, we implicitly lift $\Delta$ and $\Gamma$ to the combined alphabet $\mathcal{A}\defeq \Gamma + \Delta$. This requires some additional work for the formal proofs which we omit on paper.\mnote[preludeSig]{$\mathcal{A}$}

As in Section~\ref{sec:informal_nondet}, the initial string has the following form, where $\underline{in} = \withl \underline{\sigma} \withm \sigma \in in \withr$:
\begin{center}
  \begin{tikzpicture}[scale=1.2]
    \draw (0, 0.5) -- (10.5, 0.5);
...
Let $\initStr : \listsof{\Delta}$ be this initial string.\mnote[preludeInitialString]{$\initStr$}

For the rewrite windows, we have to ensure that all elements of $\Sigma$ are placed contiguously and that certificates of length less than $k'$ are allowed, too. Exploiting the fact that the rewrite windows overlap, this is easy to achieve.
\begin{center}
  ...
  \quad \trewwin{\nblank}{\ninit}{\nsig{\sigma_1}}{\polneut\blank}{q_0^\blank}{\sigma_1}
...
\quad \trewwin{\ninit}{\nsig{\sigma_1}}{\nblank}{q_0^\blank}{\polneut{\sigma_1}}{\polneut\blank}
\quad \trewwin{\ninit}{\nsig{\sigma_1}}{\nsig{\sigma_2}}{q_0^\blank}{\polneut{\sigma_1}}{\polneut{\sigma_2}}\\
  \trewwin{\ninit}{\nstar}{\nblank}{q_0^\blank}{\polneut{m_1}}{\polneut\blank}
  \quad \trewwin{\ninit}{\nstar}{\nstar}{q_0^\blank}{\polneut{\sigma_1}}{\polneut{m_1}}
  \quad \trewwin{\ninit}{\nstar}{\nstar}{q_0^\blank}{\polneut\blank}{\polneut\blank}\\
  \trewwin{\nsig{\sigma_1}}{\nsig{\sigma_2}}{\nsig{\sigma_3}}{\polneut{\sigma_1}}{\polneut{\sigma_2}}{\polneut{\sigma_3}} 
  ...
  \quad \trewwin{\nsig{\sigma_1}}{\nsig{\sigma_2}}{\nblank}{\polneut{\sigma_1}}{\polneut{\sigma_2}}{\polneut\blank} 
  \quad \trewwin{\nsig{\sigma_1}}{\nblank}{\nblank}{\polneut{\sigma_1}}{\polneut\blank}{\polneut\blank} \\
  \trewwin{\nstar}{\nstar}{\nstar}{\polneut{\sigma_1}}{\polneut{\sigma_2}}{\polneut{m_1}}
  \quad\trewwin{\nstar}{\nstar}{\nstar}{\polneut{\sigma_1}}{\polneut\blank}{\polneut\blank}
...
\begin{lemma}[Soundness and Completeness]\label{lem:prelude_sound_complete}\leavevmode
  \begin{enumerate}
    \coqitem[prelude_complete] $\isInitString~s \land \length{s} = l \rightarrow \initStr \strent{\Rprelude} s$
    \coqitem[prelude_sound] $\initStr \strent{\Rprelude} s \rightarrow \isInitString~s$
  \end{enumerate}
\end{lemma}

Applying Theorem~\ref{thm:expr_to_pr}, we have now arrived at the full reduction of \gennp{} to 3-\PR{}.

\begin{theorem}[Reduction of \gennp{} to \PR{}][SingleTMGenNP_to_PTPR]
...
\setCoqFilename{Undecidability.L.Complexity.Reductions.Cook.TPR_to_PR}%
Then, we do \coqlink[TPR_to_PR]{another reduction} from 3-\PR{} to \PR{}, which is just a trivial embedding.

Our definition of validity for 3-\PR{} is parameterised over an abstract $\textsf{rewritesHead} : \listsof{\Sigma} \rightarrow \listsof{\Sigma} \rightarrow \Prop$ predicate which determines whether a prefix of the first list rewrites to a prefix of the second list. The $\textsf{rewHead}$ predicate of Definition~\ref{def:rewHead} is an instance of that.
...
\setCoqFilename{Undecidability.L.Complexity.Problems.Cook.FlatPR}
As \mnote[FlatPRLang]{\textbf{FlatPR}} the finite types for the alphabet of \PR{} are not directly extractable to L, we define flat versions \textbf{FlatPR} and \textbf{Flat-3-PR} similar \setCoqFilename{Undecidability.L.Complexity.Problems.Cook.FlatTPR}
to the definition of \FlatClique{} in Chapter~\ref{chap:ksat_clique}.\mnote[FlatTPRLang]{\textbf{Flat-3-PR}}
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}[level 1/.style={sibling distance=2.7cm}, level 2/.style={sibling distance =1.3cm}, scale=0.95, every node/.style={scale=1}]
      \node {transition}
...
In order to fulfill these two requirements, we define an abstract version \mnotec[fAlphabet]{\textsf{fAlphabet}} of the alphabet $\mathcal{A}$ that contains ``holes'' where an element of $\Sigma$ or $\Sigma_{\text{state}}$, a state of $Q$, or a polarity has to be placed. 
More specifically, these holes are formalised as variables which can be of type $\Sigma, \Sigma_{\text{state}}, Q, \polarity$. \mnote[reifyAlphabetFin]{\textsf{reifyFin}}
Given an environment which provides values for the variables, we have two reification procedures \textsf{reifyFin}
and \textsf{reifyFlat} taking an element of \textsf{fAlphabet} either to a representation using the finite type $\mathcal{A}$ or a flat representation using $\nat$.\mnote[reifyAlphabetFlat]{\textsf{reifyFlat}}
...
\setCoqFilename{Undecidability.L.Complexity.Problems.Cook.GenNP}
The full reduction produces a \textbf{Flat-3-PR} instance, where the flat reification of the rewrite windows is used. As the Turing machines use finite types and vectors, which are both not directly extractable, we also have a flat encoding of these together with a flat variant \mnotec[FlatSingleTMGenNP]{\textbf{FlatTMGenNP}} of \gennp{}. 
\setCoqFilename{Undecidability.L.TM.TMflat}
The definition of flat Turing machines \textsf{TM} also represents the finite type for the tape alphabet by natural numbers and uses a list-based representation of the transition function;\ the formalisation was already available in the Coq library of undecidable problems~\cite{coq_undec}.\mnote[TM]{\textsf{TM}} 

\setCoqFilename{Undecidability.L.Complexity.Reductions.Cook.FlatSingleTMGenNP_to_FlatTPR}
\begin{theorem}[][FlatSingleTMGenNP_to_FlatTPRLang_poly]
...
\begin{center}
  \begin{tikzpicture}
    \node (flattm) at (0, 0) {\textbf{FlatTMGenNP}}; 
    \node (flatpr) at (4, 0) {\textbf{Flat-3-PR}};
    \node (tm) at (0, 2) {\gennp{}};
    \node (pr) at (4, 2) {\textbf{3-PR}};
    \path[dashed, ->, draw] (flattm) -- (tm);
    \path[dashed, ->, draw] (tm) -- (pr);
    \path[dashed, ->, draw] (pr) -- (flatpr);
    \path[->, draw] (flattm) -- (flatpr);
  \end{tikzpicture}
\end{center}
...
\setCoqFilename{Undecidability.L.Complexity.Reductions.Cook.FlatTPR_to_FlatPR}
Together with \coqlink[FlatTPR_to_FlatPR_poly]{a trivial proof} of $\textbf{Flat-3-PR} \redP{} \textbf{FlatPR}$, we obtain a reduction to $\textbf{FlatPR}$.

%\todo{comment on the definition of the flat problems, why are existentially-defined problems ugly}
