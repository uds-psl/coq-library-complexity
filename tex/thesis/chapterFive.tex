...
\begin{definition}
  \[\validInput~s \defeq \length{s} \le k \mnote{$\validInput$}\]
\end{definition}
...
Given $m$, we write $\polpos{m}$ for $(+, m)$, $\polneg{m}$ for $(-, m)$, and $\polneut{m}$ for $(\circ, m)$. For an unknown polarity $p$, we write \mnotec{$m^p$}.
...
\begin{proof}
  ...
  By assumption, there are $h_1 = \sigma_1^{\pFlipi{p}} :: \sigma_2^{\pFlipi{p}} :: h_1'$, $h_2 = \sigma_3^p :: h_2'$ with $\sigma_1 :: \sigma_2 :: left' \reprtt{z}{p} h_1$ and $\sigma_3 :: right' \reprtt{z}{p} h_2$. 
...
\begin{definition}[Initial Strings]
  \begin{align*}
    \mnotec{\initString}~s \defeq \stringForConfig~q_0~(\textsf{initTape}~s) \\
    \mnotec{\isInitString}~s \defeq \exists s', s = \initString~s' \land \validInput~s'
  \end{align*}
\end{definition}
...
\begin{problem}\label{prob:exinput}~\\
  Given an input $s$ with $\validInput{s}$, does $\PR{}~(\Gamma, 1, 3, \initString~s, \Rsim, \Rfinal, t)$ hold?
\end{problem}
...
In this section, we apply the technique presented in Section~\ref{sec:preludes} to ``guess'' an accepted input for the Turing machine. To that end, we design an initial string and nondeterministic rewrite rules that generate an input in a single rewrite step.
...
The alphabet \mnotec{$\Delta$} of the prelude has four elements, where $q_0$ is the initial state.
\[\Delta \defeq \nblank \bnfmid \nstar \bnfmid \ninit \bnfmid \ndelim \]
As in the previous section, we implicitly lift $\Delta$ and $\Gamma$ to the combined alphabet $\mathcal{A}\defeq \Gamma + \Delta$.\mnote{$\mathcal{A}$}

Following Section~\ref{sec:informal_nondet}, the initial string has the following form:
\begin{center}
  \begin{tikzpicture}[scale=1.2]
    \draw (0, 0.5) -- (10.5, 0.5);
...
For the rewrite windows, we have to ensure that all elements of $\Sigma$ are placed contiguously and that also inputs of length $< k$ are allowed. Exploiting the fact that the rewrite windows overlap, this is easy to achieve.
\begin{center}
...
\end{lemma}
\begin{proof}
  For the first statement, existence and uniqueness both follow by routine inductions on $\strent{}$. 
...
This gives us all the tools we need to prove soundness ($A_4$) and completeness ($A_3$) of the prelude.
