...
\begin{proof}
  ...
  %In the inductive step, we know that $h_1 :: h \reprt{} rs$ and $\natS{\length{h}} < z'$. 
  %The only interesting case happens when $\length{rs} \ge 2$. Therefore we assume that $rs = \sigma_1 :: \sigma_2 :: rsr$. $h = \sigma_1 :: \sigma_2 :: h'$, and since $h_1 :: h \reprt{} rs$, , 
  %making the inductive hypothesis 
  %\[\mathit{IH}: \forall a, \exists rs', rs \strent{} \polpos{a} :: rs' \land a :: \sigma_1 :: \sigma_2 :: h' \reprt{1} \polpos{a} :: rs'.\]
  %We have to show that
  %\[\forall a, \exists rs', h_1 :: \sigma_1 :: \sigma_2 :: h' \strent{} \polpos{a} :: rs' \land a :: h_1 :: \sigma_1 :: \sigma_2 :: h' \reprt{1} \polpos{a} :: rs'. \]
  %From the inductive hypothesis for 
  \TODO{} 
\end{proof}
...
\begin{definition}[\textbf{GenNP-IntermediateTape}]
...
\begin{center}
  \begin{tikzpicture}
    ...
    \path[<->] (0, -0.5) edge node [fill = white, anchor=center, pos=0.5] {\small $t + k + 2$} (5, -0.5);
    \path[<->] (8, -0.5) edge node [fill = white, anchor =center, pos=0.5] {\small $t + 2$} (10.5, -0.5);
  \end{tikzpicture}
\end{center}
...
\begin{proof}
  ...
  Now let $s_1$ be given with $s_0 \strent_{R'} s_1$. We need to show that $s_1 \in \Gamma$. 
  \TODO{}
\end{proof}
