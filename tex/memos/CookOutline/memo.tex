...
\begin{definition}[\gennp{}]
  ...
  $\exists c, (\mathit{halt}, \textsf{initTape}~x) \succ^{\le t} c$
  where 
  \begin{align*}
    \textsf{initTape}~\nil := \textsf{niltape}\\
...
Note that the second direction is equivalent to the conjunction of the first direction and the rewriting system being deterministic for strings encoding configurations. 
...
\end{definition}
\begin{theorem}[Reduction to String Rewriting]
...
\begin{definition}[Prelude]
  ...
  (where the relation $\strent{}$ is wrt.\ to the new rules $R'$) %TODO: not that it makes a difference
\end{definition}
